...
- 最终模型必须通过相同牌墙、座位和对手的配对复式评测选择。PPO 相对 BC 的
  `score_delta_95_ci` 下界必须至少为 `-1.0`，且放铳率最多增加 `1` 个百分点。
...
默认提高 8+ 番胜者样本的 policy 权重，并对稀有 win/deal-in/eight-fan 辅助标签使用
类别平衡损失。`ModelPolicy` 本身不运行大番启发式；模型通过动作级番数/结果辅助头和
`auxiliary_logit_gate` 将这些信息融合进 policy logits。
...
对已经训练完成的 BC 使用新损失微调时，使用 `--init-checkpoint` 仅加载模型权重并
重置 optimizer、scheduler 和 early-stopping：

```bash
torchrun --standalone --nproc_per_node=2 scripts/train_bc.py \
  --init-checkpoint artifacts/runs/example/bc_model.best.pt \
  --data artifacts/official_bc_v4_tensors \
  --output artifacts/runs/example/bc_model.fan-ft.pt
```
...
最佳 BC，优先比较 `qualifying_win_rate`，再比较平均分和高番子集准确率。V1/V2 checkpoint 或
cache 混用会直接报错。
...
torchrun --standalone --nproc_per_node=2 scripts/train_ppo.py \
  ...
  --updates 100 --games-per-update 16 --rollout-envs 8 --save-every 10 \
  --bc-kl-coef 0.01 --minibatch-size 1024 \
  --metrics-jsonl artifacts/runs/example/logs/ppo_metrics.jsonl
```

PPO checkpoint 会保存为 `ppo_model.update-0010.pt` 等文件。最终选模先要求相对 BC
的配对均分差 95% CI 下界至少 `-1.0` 且放铳率最多增加 `1` 个百分点，再优先选择
`qualifying_win_rate` 更高的 checkpoint。训练中的小规模联赛只用于维护 best PPO
对手，不能替代最终复式评测。
...
统一评测 BC 和所有 PPO checkpoint。通过均分与放铳护栏后，优先选择
`qualifying_win_rate` 更高的 PPO：
