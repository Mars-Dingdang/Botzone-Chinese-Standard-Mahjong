...
python scripts/preprocess_official_full_actions.py \
  --output-dir artifacts/official_bc_full_v3 --workers 8

python scripts/build_tensor_cache.py \
  --input-dir artifacts/official_bc_full_v3 \
  --output-dir artifacts/official_bc_full_v3_tensors \
...
torchrun --standalone --nproc_per_node=2 scripts/train_bc.py \
  --data artifacts/official_bc_full_v3_tensors \
  --output artifacts/runs/example/bc_model.pt \
  --epochs 15 --patience 3 --batch-size 1536 \
...
```bash
GPUS=2 BC_EPOCHS=15 PPO_UPDATES=100 EVAL_GAMES=400 \
  bash scripts/run_training_pipeline.sh start --from data
```
